const SectionCard = ({ year, title, company, description, skills, url }) => {
  const content = (
    <>
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        {skills.map((skill) => (
          <span key={skill} className="bg-cyan-300 bg-opacity-15 text-cyan-300 px-2 py-1 text-xs rounded-xl">
            {skill}
          </span>
        ))}
      </div>
  ...
  return url ? (
    ...
    <article className="mb-12 p-4 rounded-lg group">{content}</article>
  );
};
